...
def calculate_dynamic_esg_score(base_esg: float, iso_certified: str, tkdn_compliant: str, carbon_intensity: float, lead_time_days: int) -> float:
    ...
    """
    score = base_esg * 0.50
    if str(iso_certified).upper() in ["YES", "TRUE", "Y"]: score += 15.0
    if str(tkdn_compliant).upper() in ["YES", "TRUE", "Y"]: score += 15.0
    if carbon_intensity <= 0.50: score += 10.0
    elif carbon_intensity <= 0.75: score += 5.0
    if lead_time_days <= 14: score += 10.0
    elif lead_time_days <= 21: score += 5.0
